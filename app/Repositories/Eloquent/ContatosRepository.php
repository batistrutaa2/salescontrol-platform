...
class ContatosRepository implements ContatosRepositoryInterface
{
    ...
    public function __construct(
        Contatos $model,
        private readonly LeadReservatorioService $reservatorio,
        private readonly TabulationCatalog $tabulations,
    ) {
        $this->model = $model;
    }
    ...
    {
        if (Auth::check()) {
            $data['empresa_id'] = (int) app(\App\Support\TenantContext::class)->id();
        }

        return $this->model::create($data);
    }

    public function getNewlyImportedBase($idBase, int $empresaId)
    {
        return $this->model->where('id_operacao', $idBase)->where('empresa_id', $empresaId)->get();
    }

    public function all()
    {
        return $this->model->where('empresa_id', app(\App\Support\TenantContext::class)->id())->get();
    }

    public function find($id)
    {
        return $this->model->where('id', $id)->where('empresa_id', app(\App\Support\TenantContext::class)->id())->first();
    }

    public function searchForCpfsFound(array $cpfs)
    {
        $contatos = $this->model->whereIn('cpf', $cpfs)->where('empresa_id', app(\App\Support\TenantContext::class)->id())->get();
    ...
    {
        $contatos = $this->model->where('cpf', $cpf)->where('empresa_id', app(\App\Support\TenantContext::class)->id())->first();
    ...
    public function getLeads($empresa_id)
    {
        ...
        "),
            ...
            ->leftJoin('tabulacoes as c', function ($join) use ($empresa_id) {
                $join->on('b.tabulacao_id', '=', 'c.id')
                    ->where('c.empresa_id', '=', $empresa_id);
            })
            ->leftJoin('users as d', function ($join) use ($empresa_id) {
                ...
                // OU contatos que não estão em nenhum relacionamento
                    ->orWhereNotExists(function ($subquery) {
                        $subquery->select(DB::raw(1))
                            ...
                            ->whereColumn('contatos_corretores.contato_id', 'a.id')
                            ->whereColumn('contatos_corretores.empresa_id', 'a.empresa_id');
                    });
            })
            ->get();
    }
    ...
    public function quantidadeContatosImportadosMesPorVendedor($month, $year, $empresa_id)
    {
        return DB::table('contatos as a')
            ...
            ->leftJoin('users as c', function ($join) use ($empresa_id) {
                $join->on('c.id', '=', 'b.user_id')
                    ->where('c.empresa_id', '=', $empresa_id)
    ...
    public function importarLeadsAds(array $data, int $empresaId, int $userImportId)
    {
        try {
            $lead = DB::transaction(function () use ($data, $empresaId, $userImportId) {
                $lead = $this->model::create([
                    ...
                    'user_import_id' => $userImportId,
                    'id_operacao' => Helpers::generateUniqueId(),
                    'empresa_id' => $empresaId,
                    'created_at' => now(),
                    'updated_at' => now(),
                ]);
                $this->reservatorio->adicionarNovo($lead, LeadReservatorioItem::ORIGEM_MARKETING, $userImportId);
     ...
     */
    public function getResumoReciclagem(int $empresaId, int $dias, int $janelaIndicadoresDias): array
    {
        $uaSql = self::ultimaAtividadeSql($empresaId);
        ...
        $enviadosNaJanela = DB::table('preditiva_envios')
            ->where('empresa_id', $empresaId)
            ->where('enviado_em', '>=', now()->subDays($janelaIndicadoresDias))
            ->count();
        ...
        return [
            ...
            'enviados_na_janela' => $enviadosNaJanela,
            'indicadores_janela_dias' => $janelaIndicadoresDias,
        ];
    }
    ...
    {
        $filaNovos = array_values($this->tabulations->requiredIds($empresaId, [
            TabulationCode::NOVOS_CLIENTES,
            TabulationCode::PROSPECCAO,
            TabulationCode::SEM_CONTATO,
        ]));
        $uaSql = self::ultimaAtividadeSql($empresaId);
        ...
        return DB::table('contatos as a')
            ...
            ->leftJoin('tabulacoes as t', function ($join) use ($empresaId) {
