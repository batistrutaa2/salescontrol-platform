...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            <div class="d-flex justify-content-between align-items-center row gap-4 gap-md-0">
                ...
                <div class="col-md-3">
                    <button type="button" class="btn btn-primary js-transferir-leads" id="btn-transferir-massa">
    ...
    <div class="modal fade" id="modalTransferContacts" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-simple">
            <div class="modal-content">
                <div class="modal-body p-0">
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    <div class="text-center mb-6">
                        <h4 class="mb-2">Transferência em Massa</h4>
                        <p>Selecione o corretor que receberá esse lead</p>
                    </div>
                    <form id="transferLead" class="row" action="{{ route('comercial.transferContactInNulk') }}"
                        method="POST">
                        @csrf
                        <div class="col">
                            <div class="form-floating form-floating-outline">
                                <select class="select2 form-select" id="label" name="user_id" value="" required>
                                    <option value=>Selecione um corretor</option>
                                    @foreach ($users as $user)
                                        <option value="{{ $user->id }}">{{ strtoupper($user->name) }}</option>
                                    @endforeach
                                </select>
                                <label for="label">Selecionar corretor</label>
                            </div>

                            <div class="form-floating form-floating-outline mt-4">
                                <select class="select2 form-select" id="label" name="tabulation_id" required>
                                ...
                                <label for="label">Selecionar Status</label>
                            </div>

                            <!-- Input oculto para armazenar os IDs dos leads selecionados -->
                            <input type="hidden" id="selectedLeadIds" name="selectedLeadIds" value="" required>

                            <div>
                                <button class="btn btn-danger btn--twitter mt-5" id="transferButton">Transferir
                                    contato</button>
                            </div>
                        </div>
                    </form>
        ...
        <div class="modal-dialog modal-lg modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Comentários do Lead</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
