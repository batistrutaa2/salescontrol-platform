...
function brlSigned($v) {
    $v = (float)$v;
    return $v < 0 ? '− ' . brl(abs($v)) : brl($v);
}
...
// NOVO: separar vendas x ajustes para cálculo e KPIs
$linVendas  = $linhasCol->where('is_ajuste', false);
$linAjustes = $linhasCol->where('is_ajuste', true);

$hasAng = $linVendas->contains(function ($r) {
    return strtoupper((string) ($r->angariacao_status ?? '')) === 'SIM';
});

// Base total só considera VENDAS (ajustes não têm base/percentual)
...
<head>
  ...
  <style>
    /* (estilos iguais aos seus; omitidos aqui por brevidade — mantenha todos) */
    @page { margin: 20mm 12mm 16mm 12mm; }
    body { font-family: DejaVu Sans, Arial, Helvetica, sans-serif; color: #111827; font-size: 12px; }
    .gold { color: #d4af37; } .bg-gold { background-color: #d4af37; color: #111827; }
    ...
    .badge{ display:inline-block; padding:3px 8px; border:1px solid #d4af37; border-radius:999px; font-size:10px; color:#111827; margin-left:6px; background:#fffbe6;}
    .context{ display:table;width:100%;margin:4px 0 12px 0;color:#111827;}
    .ctx-col{ display:table-cell; vertical-align:middle;}
    .ctx-label{ font-size:10px;color:#6b7280;}
    .ctx-value{ font-weight:700;}
    .grid{ display:table; width:100%; table-layout: fixed; border-spacing:10px 0;}
    .col{ display:table-cell; vertical-align:top; padding:0 5px;}
    .card{ border:1px solid #d4af37; border-radius:10px; padding:10px; background:#fffef9;}
    .kpi-title{ font-size:10px; color:#6b7280; margin-bottom:2px;}
    .kpi-value{ font-size:18px; font-weight:800; color:#d4af37;}
    .kpi-note{ font-size:10px; color:#6b7280; margin-top:2px;}
    table{ width:100%; border-collapse: collapse; margin-top: 14px;}
    thead th{ background:#111827; color:#fff; font-size:11px; text-align:left; padding:8px 6px;}
    tbody td{ border-bottom:1px solid #e5e7eb; padding:7px 6px;}
    tbody tr:nth-child(even) td{ background:#f9fafb;}
    tfoot td{ border-top:2px solid #d4af37; font-weight:800; padding:8px 6px;}
    .right{text-align:right;} .center{text-align:center;}
    .section-title{ font-weight:800; margin-top:16px; margin-bottom:6px; color:#111827; border-left:4px solid #d4af37; padding-left:6px;}
    .sign{ margin-top:24px; display:table; width:100%;}
    .sign .line{ border-top:1px solid #6b7280; margin-top:40px;}
    .sign .lbl{ font-size:10px; color:#6b7280; margin-top:4px;}
    .footer{ position: fixed; left:0; right:0; bottom:-6mm; text-align:center; font-size:10px; color:#6b7280;}
    .page:after{ content: counter(page); } .pages:after{ content: counter(pages); }
    .ctx-lines .line{ margin-top:2px;} .ctx-lines .line+.line{ margin-top:4px;}
    .ctx-chip{ display:inline-block; padding:2px 6px; border:1px solid #d4af37; border-radius:999px; font-size:10px; background:#fffbe6; color:#111827; margin-left:6px;}
  </style>
</head>
<body>
  ...
  <!-- NOVO: Quebra VENDAS x AJUSTES -->
  <div class="grid">
    <div class="col" style="width: 25%;">
      <div class="card">
        <div class="kpi-title">Vendas (Líquido)</div>
        <div class="kpi-value">{{ brl($totVendas['liquido'] ?? 0) }}</div>
        <div class="kpi-note">Somente contratos</div>
      </div>
    </div>
    <div class="col" style="width: 25%;">
      <div class="card">
        <div class="kpi-title">Ajustes (Créditos)</div>
        <div class="kpi-value">{{ brl($totAjustes['creditos'] ?? 0) }}</div>
        <div class="kpi-note">Incrementam o líquido</div>
      </div>
    </div>
    <div class="col" style="width: 25%;">
      <div class="card">
        <div class="kpi-title">Ajustes (Débitos)</div>
        <div class="kpi-value">{{ brl($totAjustes['debitos'] ?? 0) }}</div>
        <div class="kpi-note">Reduzem o líquido</div>
      </div>
    </div>
    <div class="col" style="width: 25%;">
      <div class="card">
        <div class="kpi-title">Ajustes (Líquido)</div>
        <div class="kpi-value">{{ brlSigned($totAjustes['liquido'] ?? 0) }}</div>
        <div class="kpi-note">Créditos − Débitos</div>
      </div>
    </div>
  </div>
  ...
  <table>
    <thead>
      <tr>
        ...
        <th class="center">Tipo</th> <!-- NOVO: mostra Venda/Angariação/Ajuste -->
      </tr>
    </thead>
    <tbody>
      @forelse ($linhas as $r)
        ...
        <tr>
          <td class="center">
            @if ($isAdj)
              —
            @else
              {{ \Carbon\Carbon::parse($r->data_implantacao)->format('d/m/Y') }}
            @endif
          </td>
          <td>
            {{ $r->nome_contrato }}
