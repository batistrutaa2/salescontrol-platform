...
Esta tela segue o **SalesControl UI**, documentado em `CLAUDE.md` (seção Design System) e implementado em `resources/assets/vendor/scss/pages/dashboard-analytics.scss`. A referência de aplicação é o Cofre de Acessos (`resources/views/content/pages/backoffice/credenciais/index.blade.php`). A direção Temporada Comercial descrita acima pertence ao dashboard de desempenho; não substitui o design system das telas operacionais.

- Reutilizar `table-card`, `table-header`, `table-title-group`, `table-icon` e os tokens `--dash-*`.
- Tipografia Plus Jakarta Sans; valores monetários em JetBrains Mono. Cards com raio do token de 16px, sombras, gradientes e superfícies do sistema.
- Filtros em painel compacto, ações com tamanho de conteúdo e registros com separadores. Não esticar botões por toda a largura no desktop.
- Preservar layout compartilhado, largura útil, tema `.dark-style`, SVG inline e foco visível. Reorganizar filtros e registros no celular.
- Manter alertas de 10 dias, filtros por vencimento, cadastros, edição, exclusão e histórico, sem mudar regras operacionais por motivo visual.
